...
public class MainActivity extends AppCompatActivity {
    ...
    public void displayPrice() {
        ...
        taxTextView.setText(NumberFormat.getCurrencyInstance().format(calculateTax(quantity, taxValue)) + " tax");
    ...
    public void sendEmailOrderSummary(String address, String bodyText) {
        ...
        if(nameEditText.getText().toString().length() < 1){
            Toast.makeText(this, "Enter a valid name to submit order.", Toast.LENGTH_SHORT).show();
        }
        else {
            Intent intent = new Intent(Intent.ACTION_SENDTO);
            intent.setData(Uri.parse("mailto:")); // only email apps should handle this
            intent.putExtra(Intent.EXTRA_EMAIL, address);
            intent.putExtra(Intent.EXTRA_SUBJECT, getString(R.string.app_name) + " " + getString(R.string.order) + " from " + nameEditText.getText().toString());
